...
from tensorflow.keras.layers import Input, Add, Activation, Lambda, MaxPooling2D, Reshape, SeparableConv2D,LayerNormalization
...
def channel_spatial_squeeze_excite_model(input_size, d_model):
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=channel_spatial_squeeze_excite(cnn)
...
def spatial_squeeze_excite_block_model(input_size, d_model):
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=spatial_squeeze_excite_block(cnn)
    cnn = FullGatedConv2D(filters=8, kernel_size=(3, 3), padding="same")(cnn)
...
def NoAtt_model(input_size, d_model):
    """
    No attention added
    """
...
from network.gc import global_context_block
def GCNet_model(input_size, d_model):

    input_data = Input(name="input", shape=input_size)
    #cnn=PixelAttention2D(input_data.shape[-1])(input_data)
    cnn = Conv2D(filters=16, kernel_size=(3, 3), strides=(1,1), padding="same", kernel_initializer="he_uniform",activation='selu')(input_data)
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=global_context_block(cnn)
    cnn = FullGatedConv2D(filters=8, kernel_size=(3, 3), padding="same")(cnn)
...
def SeNet_model(input_size, d_model):
...
def cbam_nospatial_model(input_size, d_model):
    """
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=8, kernel= 5,spatial= False,name="block1")
    cnn = FullGatedConv2D(filters=8, kernel_size=(3, 3), padding="same")(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=16, kernel= 5,spatial= False,name="block2")
    cnn = FullGatedConv2D(filters=16, kernel_size=(3, 3), padding="same")(cnn)
    #cnn=PixelAttention2D(cnn.shape[-1])(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=24, kernel= 5,spatial= False,name="block3")
    cnn = FullGatedConv2D(filters=24, kernel_size=(3, 3), padding="same")(cnn)
    #cnn=PixelAttention2D(cnn.shape[-1])(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=32, kernel= 5,spatial= False,name="block4")
    cnn = FullGatedConv2D(filters=32, kernel_size=(3, 3), padding="same")(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=40, kernel= 5,spatial= False,name="block5")
    cnn = FullGatedConv2D(filters=40, kernel_size=(3, 3), padding="same", kernel_constraint=MaxNorm(4, [0, 1, 2]))(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=48, kernel= 5,spatial= False,name="block6")
    cnn = FullGatedConv2D(filters=48, kernel_size=(3, 3), padding="same", kernel_constraint=MaxNorm(4, [0, 1, 2]))(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=56, kernel= 5,spatial= False,name="block7")
    cnn = FullGatedConv2D(filters=56, kernel_size=(3, 3), padding="same", kernel_constraint=MaxNorm(4, [0, 1, 2]))(cnn)
...
def cbam_model(input_size, d_model):
    """
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=8, kernel= 5,spatial= True,name="block1")
    cnn = FullGatedConv2D(filters=8, kernel_size=(3, 3), padding="same")(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=16, kernel= 5,spatial= True,name="block2")
    cnn = FullGatedConv2D(filters=16, kernel_size=(3, 3), padding="same")(cnn)
    #cnn=PixelAttention2D(cnn.shape[-1])(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=24, kernel= 5,spatial= True,name="block3")
    cnn = FullGatedConv2D(filters=24, kernel_size=(3, 3), padding="same")(cnn)
    #cnn=PixelAttention2D(cnn.shape[-1])(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=32, kernel= 5,spatial= True,name="block4")
    cnn = FullGatedConv2D(filters=32, kernel_size=(3, 3), padding="same")(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=40, kernel= 5,spatial= True,name="block5")
    cnn = FullGatedConv2D(filters=40, kernel_size=(3, 3), padding="same", kernel_constraint=MaxNorm(4, [0, 1, 2]))(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=48, kernel= 5,spatial= True,name="block6")
    cnn = FullGatedConv2D(filters=48, kernel_size=(3, 3), padding="same", kernel_constraint=MaxNorm(4, [0, 1, 2]))(cnn)
    ...
    cnn = BatchNormalization(renorm=True)(cnn)
    #cnn=squeeze_excite_block(cnn)
    cnn=cbam_block(input_tensor=cnn,features=56, kernel= 5,spatial= True,name="block7")
    cnn = FullGatedConv2D(filters=56, kernel_size=(3, 3), padding="same", kernel_constraint=MaxNorm(4, [0, 1, 2]))(cnn)
...
def EfficientGated_HTR(input_size, d_model):
...
def flor_withual_attention(input_size, d_model):
...
def ECA5_model(input_size, d_model):
    """
    ECA-Net with K=5 attention added
    """
...
def ECA3_model(input_size, d_model):
...
def flor_withSeNET_added(input_size, d_model):
...
def dual_attention_model(input_size, d_model):
...
class ECA1(tf.Module):
  ...
  def __call__(self, x):
    ...
    attn = tf.keras.layers.Conv1D(filters=1, kernel_size=k_size, padding='same', 
...
def channel_spatial_squeeze_excite(input, ratio=4):
    ''' Create a spatial squeeze-excite block
    Args:
        input: input tensor

...
def cbam_block(
    ...
    # Unique names for channel and spatial attention models
    channel_attention_name = name + "_chn"
    spatial_attention_name = name + "_spt"

    out_tensor = channel_attention(features, name=channel_attention_name)(input_tensor)
    if spatial:
        out_tensor = spatial_attention(features, kernel=kernel, name=spatial_attention_name)(
            out_tensor
        )
    return out_tensor
